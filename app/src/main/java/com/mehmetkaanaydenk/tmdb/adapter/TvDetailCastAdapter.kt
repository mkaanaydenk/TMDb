...
class TvDetailCastAdapter(val castList: List<Cast>) :
    RecyclerView.Adapter<TvDetailCastAdapter.CastHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CastHolder {
        val binding =
            DetailCastRowBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: CastHolder, position: Int) {
        holder.binding.castImageview.downloadUrl(
            IMAGE_BASE_URL + castList[position].profilePath,
